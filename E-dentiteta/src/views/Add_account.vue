      ...
      <div class="login">
        <div class="login_form">
          ...
          <ion-input
            ...
            required
          />
          <button class="login_button" @click="register()">Registracija</button>
        </div>
      </div>
    </ion-content>
  </ion-page>
...
export default defineComponent({
  ...
  created() {
    ...
    axios.post(
      "https://e-dentiteta.usdd.company/APIs/add_acc.php",
      formData,
      {}
    );
  },
  methods: {
    async register() {
      if (
        this.username != "" &&
        this.password != "" &&
        this.password_re == this.password
      ) {
        const formData = new FormData();
        formData.append("username", this.username);
        formData.append("password", this.password);
        const response = await axios.post(
          "https://e-dentiteta.usdd.company/APIs/register.php",
          formData
        );
        ...
        } else if (response.data === "Expired link.") {
          const alert = await alertController.create({
            header: "Povezava je potekla",
            message:
              "Povezava za registacijo je potekla. Prosimo, posvetiujte se z sistemskim administratorjem.",
            buttons: ["Zapri"],
          });
          await alert.present();
